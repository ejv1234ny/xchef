...
## 9. LLM provider switch to OpenAI + PDF previews (added 2026-09-04)

`lib/llm/provider.ts` exposes one `structured()` call used by invoice parse, recipe draft, SKU match and sheet map; `LLM_PROVIDER` picks `openai` (default) or `anthropic`. The OpenAI implementation (`lib/llm/openai.ts`) uses the Responses API with strict JSON-schema output — zod schemas converted by `lib/llm/strict-schema.ts` (every property required, optionals nullable and stripped back to absent, unsupported keywords removed) — temperature 0, images as `input_image` data URLs, PDFs as `input_file`, text inline. Models pinned in `lib/llm/models.ts`: gpt-4.1 for invoice parsing and recipe drafting, gpt-4.1-mini for SKU and sheet mapping; tokens and list-price cost logged to `llm_calls` with the new `provider` column (migration 0008). The Claude path is unchanged behind the same interface. Tests: strict-schema conversion validated for all four schemas; the OpenAI provider exercised with mocked HTTP for invoice parse (PDF and image), SKU match, recipe draft, sheet map, and refusal handling. `Retry parse` appears on review pages for `needs_review` documents with no lines; `pnpm invoices:reparse --id <uuid>` does the same from the CLI.

PDF previews: `lib/pdf-preview.ts` renders page 1 with pdfjs-dist + @napi-rs/canvas at upload time (stored as `<hash>.preview.png` next to the original; rendered lazily on first view for older documents); signed URLs are created with download disabled and objects are stored with their real content type. The review page embeds the PDF inline on tablets/desktop, shows the rendered page on phones, and always offers "Open PDF". HEIC/HEIF photos are converted to JPEG at intake (`heic-convert`) so browsers and the parser can read them.

### Verified 2026-09-04 13:05 UTC — OpenAI in production

- `pnpm invoices:reparse --id a8534817-…` (PFG statement forwarded by email): gpt-4.1 returned `document_kind: statement` → document `rejected`, "not an invoice"; `llm_calls` row provider=openai, 1,982 in / 100 out tokens, $0.0048.
- Production, via the Resend webhook: a real fixture invoice emailed to `invoices@istieso.resend.app` → `inbound_events` 0d8d74ce → document 5424552d (802 Spirits, invoice 2017-2017-1-176, 12 lines, confidence 0.97) parsed by gpt-4.1 ($0.0123) and mapped by gpt-4.1-mini (sku-match calls at $0.0005–0.0006 each), all with provider=openai. The parser flagged "line total 251.88 vs subtotal 390.85" for human review.
- A first test email was correctly deduplicated against the same PDF uploaded through the app minutes earlier (content hash match → no new document, event logged with `documents_created = 0`).
- `LLM_PROVIDER=openai`, `OPENAI_API_KEY`, `INBOUND_EMAIL_ADDRESS=invoices@istieso.resend.app` are set on Vercel (production + preview).
- 13:10 UTC, deployment `251ac5e`: a third emailed fixture (invoice802-1.pdf) → document b462a783 (802 Spirits, 14 lines, 10 to review), parsed by gpt-4.1 ($0.0138) with gpt-4.1-mini SKU matches, and its `<hash>.preview.png` rendered by pdfjs inside the Vercel function (an earlier attempt failed because the pdfjs worker was not traced into the bundle; fixed by importing it explicitly — note: `outputFileTracingIncludes` into pnpm's symlinked store breaks Vercel packaging, so it is not used).


## 10. Multi-receipt scans and retail liquor receipts (added 2026-09-04)

- Parser returns `documents[]`; each receipt/invoice on a page becomes its own `invoice_documents` row (same `storage_path`, region in `raw_extraction`). Per document: `receipt_id` (barcode / receipt number, else invoice number), `transaction_code` (non-unique register codes such as "2017-2017-1-176"), `invoice_time`, `printed_item_count`. Discount/promo/credit lines under an item are that item's `adjustment` (extended = gross − adjustment) and are never lines; deposit sections become `deposit` lines excluded from the subtotal check. Migration 0009 adds these columns, `vendors.kind`, and the dedupe indexes.
- Scanned PDFs (no text layer) are rendered by pdfjs and sent as a page image plus six high-resolution crops; text PDFs still go as PDFs. Every document is validated (Σ lines ≈ subtotal ±2% and line count or quantity sum = printed count); the model is re-asked with the discrepancy when a document fails, and once more when a sum is off by cents (a dropped discount). The best attempt wins.
- Dedupe per receipt: same vendor + receipt id (exact or shared suffix, since barcode prefixes get cut off between scans), else same vendor + date + time + total. Duplicate receipts inside a re-scanned file are skipped; a file whose receipts all exist elsewhere is deleted.
- Retail liquor (`vendors.kind = 'retail_liquor'`, 802 Spirits set): every line is bottles; `lib/core/liquor.ts` infers 750ML / 1L / 1.75L / 375ML / 50ML / n-pack sizes from the name, defaults to 750 ml with `pack_size_assumed = true`; the review page shows "(assumed)" and offers one-tap size buttons that also fix the vendor mapping.
- Tests: `invoice802-1.pdf` → 2 documents; left 10 lines (quantity 15 = "Total Sales Quantity 15"), Σ exactly 390.85 with $31.00 of "Offer disc." applied; right 7 lines, Σ exactly 203.93. Coca-Cola distributor scan → 1 document, 8 lines, Σ 733.97 (group headers and the deposit section excluded). Unit tests cover normalization, validation and bottle-size inference (115 tests + 2 real-model fixture tests).
- Database after reparse: exactly two 802 Spirits documents for that page — b462a783 (left, 2025-12-21 12:21, receipt 201717-1155731) and 5424552d (right, 2025-12-19 16:47, receipt …1155526); the duplicate halves from the second scan were removed. The model still misreads faint subtotal digits (390.05 / 203.98 printed as read); the line sums are the reliable numbers and the review screen shows both.
