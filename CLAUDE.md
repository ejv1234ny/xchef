...
8. Every intake path (email, forward, upload, paste, manual) creates an `invoice_documents` row and goes through parse → map → post. Post only when every line is mapped or ignored. Statements → rejected. Credits → negative quantities.
